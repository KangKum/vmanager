...
const HighPage = () => {
  const [highClasses, setHighClasses] = useState<IClassTable[]>([]);
  const [nextId, setNextId] = useState(0);

  const handleDeleteClass = (id: number) => {
    if (confirm("수업을 삭제하시겠습니까?")) setHighClasses((prev) => prev.filter((classItem) => classItem.tableId !== id));
  };

  const handleLevelChange = (tableId: number, newLevel: number) => {
    setHighClasses((prev) => prev.map((classItem) => (classItem.tableId === tableId ? { ...classItem, currentLevel: newLevel } : classItem)));
  };
  ...
  const handleSort = () => {
    ...
    setNextId(highClasses.length);
  };

  // isSorted가 true일 때만 정렬
  const displayClasses = highClasses;
  ...
  return (
    <div className="w-full flex flex-col">
      <button
        ...
        onClick={() => {
          setHighClasses((prev) => [...prev, { tableId: nextId, currentLevel: 1 }]);
